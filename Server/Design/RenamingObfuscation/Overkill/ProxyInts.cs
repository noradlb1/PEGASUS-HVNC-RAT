...
namespace PEGASUS.Design.RenamingObfuscation.Overkill
{
    internal class ProxyInts
    {
        ...
        {
            var ManifestModule = Ovelkill.Module;
                ...
                foreach (var method in type.Methods)
                {
                    ...
                    for (var i = 0; i < instr.Count; i++)
                        if (method.Body.Instructions[i].IsLdcI4())
                        {
                            ...
                            var methFlags = MethodAttributes.Public | MethodAttributes.Static |
                        ...
                        else if (method.Body.Instructions[i].OpCode == OpCodes.Ldc_R4)
                        {
                            ...
                            meth1.Body.Instructions.Add(Instruction.Create(OpCodes.Ldc_R4,
                                (float) method.Body.Instructions[i].Operand));
                            meth1.Body.Instructions.Add(Instruction.Create(OpCodes.Ret));
                            instr[i].OpCode = OpCodes.Call;
                            instr[i].Operand = meth1;
                            Amount++;
