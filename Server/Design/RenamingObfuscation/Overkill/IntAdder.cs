...
namespace PEGASUS.Design.RenamingObfuscation.Overkill
{
    public class AddInteger
    {
        ...
        {
            foreach (var type in Ovelkill.Module.GetTypes())
            {
                ...
                foreach (var methodDef2 in type.Methods)
                {
                    ...
                    for (var i = 0; i < instr.Count; i++)
                        if (methodDef2.Body.Instructions[i].IsLdcI4())
                        {
                            ...
                            methodDef2.Body.Instructions.Insert(i + 1,
                                Instruction.Create(OpCodes.Sizeof, methodDef2.Module.Import(typeof(bool))));
                            ...
                            methodDef2.Body.Instructions.Insert(i + 4,
                                Instruction.Create(OpCodes.Call,
                                    methodDef2.Module.Import(typeof(Math).GetMethod("Sin", new[] {typeof(double)}))));
                            ...
                            methodDef2.Body.Instructions.Insert(i + 7,
                                Instruction.Create(OpCodes.Sizeof, methodDef2.Module.Import(typeof(bool))));
                            methodDef2.Body.Instructions.Insert(i + 8, Instruction.Create(OpCodes.Add));
                            methodDef2.Body.Instructions.Insert(i + 9,
                                Instruction.Create(OpCodes.Ldc_R8, randomuint ^ randomuint));
                            methodDef2.Body.Instructions.Insert(i + 10,
                                Instruction.Create(OpCodes.Call,
                                    methodDef2.Module.Import(typeof(Math).GetMethod("Cos", new[] {typeof(double)}))));
